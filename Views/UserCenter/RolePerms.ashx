<%@ WebHandler Language="C#" Class="RolePerms" %>
...
public class RolePerms : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string flag = MicroPublic.GetMsg("DenyUrlParaError"),
             ...
             RID = context.Server.UrlDecode(context.Request.Form["id"]),
        ...
        try
        {
            if (!string.IsNullOrEmpty(Action))
            {
                ...
                if (Action == "getroleperms" && !string.IsNullOrEmpty(RID))
                    flag = GetMainMenu(RID);

                if (Action == "modify" && !string.IsNullOrEmpty(RID) && !string.IsNullOrEmpty(Fields))
                    flag = ModifyRolePermissions(RID, Fields);
    ...
    /// 显示主模块和可操作权限
    /// </summary>
    /// <param name="RID">角色权限</param>
    /// <returns></returns>
    private string GetMainMenu(string RID)
    {
        ...
        string _sql3 = "select MPID, RID from RolePermissions ";
        ...
        if (_dt != null && _dt.Rows.Count > 0)
        {
            ...
            foreach (DataRow _dr in _rows)
            {
                ...
                _str += GetSubMenu(ModuleID, _dt, _dt2, _dt3, RID); //查询子模块
                _str += "}";
    ...
    /// <param name="_dt3">RolePermissions</param>
    /// <param name="RID">角色ID</param>
    /// <returns></returns>
    private string GetSubMenu(string MID, DataTable _dt, DataTable _dt2, DataTable _dt3, string RID)
    {
        string flag = string.Empty, _str = string.Empty;
        ...
        _str += GetModulePerms(MID, _dt, _dt2, _dt3, RID);
        ...
        if (_rows.Length > 0)
        {
            ...
            foreach (DataRow _dr in _rows)
            {
                ...
                _str += GetSubMenu(ModuleID, _dt, _dt2, _dt3, RID); //递归
                _str += "}";
    ...
    /// <param name="RID">角色ID</param>
    /// <returns></returns>
    private string GetModulePerms(string MID, DataTable _dt, DataTable _dt2, DataTable _dt3, string RID)
    {
        string flag = string.Empty, _str = string.Empty;
        DataRow[] _rows = _dt.Select("ParentID=" + MID, "ParentID,Sort");
        ...
        if (_rows2.Length > 0)
        {
            ...
            foreach (DataRow _dr2 in _rows2)
            {
                ...
                DataRow[] _rows3 = _dt3.Select("MPID = " + MPID + " and RID= " + RID);
                if (_rows3.Length > 0)
    ...
    /// <returns></returns>
    private string ModifyRolePermissions(string RID, string Fields)
    {
        string flag = string.Empty;

        flag = GetMainCheckedData(Fields, RID);
    ...
    /// <param name="RID">角色ID</param>
    /// <returns></returns>
    private string GetMainCheckedData(string Fields, string RID)
    {
        ...
        _dt.Columns.Add("RID", typeof(int));

        //查询角色权限表，与当前选中权限作匹配，已有的权限不作改变，没有的权限取出来，最后插入数据库
        string _sql2 = "select MPID, RID from RolePermissions ";
        DataTable _dt2 = MsSQLDbHelper.Query(_sql2).Tables[0];
        ...
        for (int i = 0; i < iArray.Count; i++)
        {
            ...
            if (!string.IsNullOrEmpty(iArray[i]["field"].ToString()))
            {
                ...
                //当数据库没有记录时再作插入操作
                DataRow[] _rows2 = _dt2.Select("MPID=" + MPID + " and RID=" + RID.toInt());
                if (_rows2.Length == 0)
                {
            ...
            if (iArray[i]["children"] != null)
            {
                ...
                if (jArray.Count > 0)
                    GetSubCheckedData(jArray, _dt, _dt2, RID, SB);
            }
            Num = Num + 1;
        }
        ...
        if (Num > 0)
        {
            ...
            if (_dt != null && _dt.Rows.Count > 0)
                MsSQLDbHelper.SqlBulkCopyInsert(_dt, "RolePermissions");
            ...
            if (MPID.Length > 0)
            {
                ...
                string _sqlDel = "delete RolePermissions where RID=@RID and MPID not in (" + MPID + ")";
                SqlParameter[] _spDel = { new SqlParameter("@RID", SqlDbType.Int) };
                _spDel[0].Value = RID.toInt();
    ...
    /// <param name="_dt"></param>
    /// <param name="RID"></param>
    private void GetSubCheckedData(JArray jArray, DataTable _dt, DataTable _dt2, string RID, StringBuilder SB)
    {
        ...
        for (int j = 0; j < jArray.Count; j++)
        {
            ...
            if (jArray[j]["children"] != null)
            {
                ...
                if (jArray2.Count > 0)
                    GetSubCheckedData(jArray2, _dt, _dt2, RID, SB);
            }
        }
