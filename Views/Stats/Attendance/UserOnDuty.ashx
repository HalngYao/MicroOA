<%@ WebHandler Language="C#" Class="UserOnDuty" %>
...
public class UserOnDuty : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string flag =  "{\"code\": 0,\"msg\": \"数据读取失败<br/>データの読み込みに失敗しました<br/>Data reading failed\",\"count\":0,\"cols\":  [],\"data\":  [] }",
                    Action = context.Server.UrlDecode(context.Request.Form["Action"]),
                    ModuleID = context.Server.UrlDecode(context.Request.Form["ModuleID"]),
                    ...
                    Keyword = context.Server.UrlDecode(context.Request.Form["Keyword"]),
                    StartDate = context.Server.UrlDecode(context.Request.Form["StartDate"]),
                    EndDate = context.Server.UrlDecode(context.Request.Form["EndDate"]),
                    Page = context.Server.UrlDecode(context.Request.Form["page"]),
                    Limit = context.Server.UrlDecode(context.Request.Form["limit"]);

        ////测试数据
        //Action = "View";
        //ShortTableName = "OnDuty";
        //ModuleID = "37";
        ...
        context.Response.Write(JObject.Parse(flag));
    }
    ...
    private string GetUserOvertime(string ModuleID, string ShortTableName, string DataType = "", string QueryType = "Dept", string Keyword = "All", string StartDate = "", string EndDate = "", string Page = "1", string Limit = "10")
    {
        ...
        string Cols = "{ type: 'checkbox', fixed: 'left'}" +
                    ...
                    ", { field: 'ColUserWorkHourSystem', title: '工时制', width: 100, align: 'center', sort: true }";
        ...
        if (_dt != null && _dt.Rows.Count > 0)
        {
            ...
            DataTable _dtUserDepts = MicroDataTable.GetAllUserAttrDataTable("UserDepts"),
                    _dtUserJobTitle = MicroDataTable.GetAllUserAttrDataTable("UserJobTitle"),
                    ...
                    _dtUserOnDuty = MicroDataTable.GetAllUserAttrDataTable("UserOnDuty", StartDate, EndDate),
            ...
            var getDateTimeDiff = MicroPublic.GetDateTimeDiff(StartDate.toDateFormat("yyyy-MM-dd"), EndDate.toDateFormat("yyyy-MM-dd"));
            ...
            for (int i = 0; i < _dt.Rows.Count; i++)
            {
                ...
                {
                    foreach (DataRow _dr in _rowsUserDepts)
                    {
                        DeptName += _dr["DeptName"].toStringTrim() + "/";
                    }
                ...
                {
                    string CurrDate = StartDate.toDateTime().AddDays(j).ToString("yyyy-MM-dd"),
                         CurrDateStart = CurrDate + " 00:00:00.000",
                            CurrDateEnd = CurrDate + " 23:59:59.998";

                    DataRow[] _rowsUserOnDuty = _dtUserOnDuty.Select("DutyUID=" + UserID.toInt() + " and DutyDate='" + CurrDate + "'", "DutyDate desc");

                    //判断是否为休息日
                    Boolean IsOffDay = false;
                    DataRow[] _rowsCalendarDays2 = _dtCalendarDays.Select("DayDate='" + CurrDate + "'");
                    if (_rowsCalendarDays2.Length > 0)
                        IsOffDay = true;

                    //如果排班申请里有申请记录
                    if (_rowsUserOnDuty.Length > 0)
                    {
                        ShiftName = _rowsUserOnDuty[0]["ShiftName"].toStringTrim();
                        ...
                        if (ShiftName == "-")
                        {
                            ...
                            if (!IsOffDay)
                            {
                                //如果取消了排班申请，判断你是否是武汉的，如果不是则是广州的
                                DataRow[] _rowsUserDepts2 = _dtUserDepts.Select("UID=" + UserID.toInt() + " and DeptID=8");
                                if (_rowsUserDepts2.Length > 0)
                                    ShiftName = _dtShiftType.Select("ShiftTypeID=6")[0]["Alias"].toStringTrim();
                                else
                                    ShiftName = _dtShiftType.Select("ShiftTypeID=2")[0]["Alias"].toStringTrim();
                            }
                            else
                                ShiftName = "-";
                        ...
                        {
                            DataRow[] _rowsUserLeave2 = _dtUserLeave.Select("LeaveUID=" + UserID.toInt() + " and StartDateTime<='" + CurrDateEnd + "' and EndDateTime>='" + CurrDateStart + "'");
                            //如果提交了休假申请时
                            if (_rowsUserLeave2.Length > 0)
                            {
                                string HolidayName = string.Empty;
                                foreach (DataRow _dr in _rowsUserLeave2)
                                {
                                    HolidayName += _dr["HolidayName"].toStringTrim() + "、";
                                }
                                if (!string.IsNullOrEmpty(HolidayName))
                                    HolidayName = "（" + HolidayName.Substring(0, HolidayName.Length - 1) + "）";

                                ShiftName = "休" + HolidayName;
                            }
                            else
                                ShiftName = "休（未提交休假申请）";
                        }
                    ...
                    OnDutyStr += ",\"" + CurrDate + "\":\"" + ShiftName + "\"";    //日期:班次

                    //########生成表头########
                    //仅只读取一次用于生成表头，注意是i=0，而不是j=0
                    if (i == 0)
                    {
                        string Week = MicroPublic.GetWeek(CurrDate, "WW"),
                        OffDay = string.Empty;

                        DataRow[] _rowsCalendarDays = _dtCalendarDays.Select("DayDate= '" + CurrDate + "'");
                        if (_rowsCalendarDays.Length > 0)
                        {
                            string DaysType = _rowsCalendarDays[0]["DaysType"].ToString();
                            if (DaysType == "OffDay")
                                OffDay = " 休";
                        ...
                        //动态构造表头
                        Cols += ",{field: '" + CurrDate + "', title: '" + CurrDate + Week + "', width:200, align:'center', sort:true }";
                    }
                    //########生成表头########
                }
        ...
        flag = "{\"code\": 0,\"msg\": \"True加载成功\",\"count\": " + TotalCount + ",\"cols\":  [" + Cols + "],\"data\":  [" + flag + "] }";
    ...
    {
        string strTpl = "\"UID\":\"{0}\", \"UserName\":\"{1}\",\"EMail\":\"{2}\",\"ColUserDepts\":\"{3}\", \"DisplayName\":\"{4}\", \"ColUserJobTitle\":\"{5}\", \"WorkTel\":\"{6}\", \"WorkMobilePhone\":\"{7}\", \"ColUserWorkHourSystem\":\"{8}\", \"ShiftName\":\"{9}\", \"WorkOvertime\":\"{10}\", \"OffdayOvertime\":\"{11}\", \"StatutoryOvertime\":\"{12}\", \"OvertimeTotal\":\"{13}\", \"AlreadyDaiXiu\":\"{14}\", \"ExceptDaiXiu\":\"{15}\", \"WarningValue\":\"{16}\", \"AnnualLeave\":\"{17}\", \"LeaveTotal\":\"{18}\"";
