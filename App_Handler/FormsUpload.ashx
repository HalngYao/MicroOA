<%@ WebHandler Language="C#" Class="FormsUpload" %>
...
public class FormsUpload : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
        ...
        if (((MicroUserInfo)context.Session["userInfo"]).GetIsLogin())
        {
            try
            {
                ...
                if (files.Count > 0)
                {
                    ...
                    fileNewName = DateTime.Now.ToString("yyyyMMddHHmmssff") + "_" + fileName;
                    //保存文件
                    files[0].SaveAs(context.Server.MapPath("~/Resource/UploadFiles/Forms/" + fileNewName));
                    filePath = "/Resource/UploadFiles/Forms/" + fileNewName;
                    msg = "文件上传成功！";
                    ...
                    result = "{\"code\": " + 0 + ", \"msg\": \"" + msg + "\", \"data\": { \"src\": \"" + filePath + "\", \"title\":\"" + fileName.Split('.')[0] + "\"}}";
                }
                else
                {
                    error = "文件上传失败！";
